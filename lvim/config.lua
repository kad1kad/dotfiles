...
formatters.setup({
	...
	},
	{
		...
		extra_args = {
			...
			"--quote-style",
			"AutoPreferDouble", -- Prefer double quotes if possible
		},
	},
})
...
lvim.format_on_save.pattern = { "*.lua", "*.scss", "*.html" }
...
lvim.keys.normal_mode["<leader>s"] = "ciw"
lvim.keys.normal_mode["<leader>a"] = "ggVG"
lvim.keys.normal_mode["<BS>"] = "b"
...
lvim.builtin.telescope.defaults.vimgrep_arguments = {
	...
	"--glob",
	"!*.min.*", -- exclude files with `.min.` in their names
}
